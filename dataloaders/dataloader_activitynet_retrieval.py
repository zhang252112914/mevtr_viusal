...
class ActivityNet_DataLoader(Dataset):
    def __init__(
        ...
        pseudo_video_id_list, _ = self._get_video_id_single(video_id_path_dict[self.subset])
        video_id_list = pseudo_video_id_list
        ...
        self.video_id2idx_dict = {pseudo_video_id: id_ for id_, pseudo_video_id in enumerate(self.pseudo_video_id_list)}
        # Get all captions
...
class ActivityNet_DataLoader_2(Dataset):
    def __init__(
            self,
            subset,
            data_path,
        ...
        # Get iterator video ids
        self.video_id2idx_dict = {video_id: id_ for id_, video_id in enumerate(self.video_id_list)}
        # Get all captions
        self.full_data = []
        with open(video_json_path_dict[self.subset], 'r') as f:
            data = json.load(f)
        for key in data.keys():
            len_ = len(data[key]['timestamps'])
            if key not in video_dict:
                continue
            for j in range(len_):
                d = {'video_id': key, 'start_time': int(data[key]['timestamps'][j][0]),
                     'end_time': int(data[key]['timestamps'][j][1]), 'sentence': data[key]['sentences'][j]}
                self.full_data.append(d)
    ...
    def _get_text(self, dat):
        ...
        words = self.tokenizer.tokenize(dat['sentence'])
    ...
    def _get_rawvideo(self, idx, s, e):
        ...
        try:
            ...
            # Should be optimized by gathering all asking of this video
            raw_video_data = self.rawVideoExtractor.get_video_data(video_path, start_time, end_time)['video']
    ...
    def __getitem__(self, feature_idx):
        dat = self.full_data[feature_idx]
        pairs_text, pairs_mask, pairs_segment = self._get_text(dat)
        video, video_mask = self._get_rawvideo(dat['video_id'], dat['start_time'], dat['end_time'])
        return pairs_text, pairs_mask, pairs_segment, video, video_mask
