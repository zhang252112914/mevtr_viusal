...
class CharadesMeDataloader(Dataset):
    ...
    def __init__(
        ...
        with open(fname, 'r') as f:
            L = f.readlines()
        L = [x.strip().split('##') for x in L] # explain strip(): 
        desc = [x[1] for x in L]
    ...
    def _get_text(self, sentences):
        ...
        # if n is les than k, then pad the sentence with 0, to make the length of expand_sentences equal to k.
        if n < k:
            expand_sentences = expand_sentences + [""] * (k - n)

        return pairs_text, pairs_mask, group_mask, expand_sentences
    ...
    def _get_rawvideo(self, video_path, dur, s, e):
        ...
        return video, video_mask, path

    # Reture a mask of shape (max_text_per_video, max_frames)
    # 1 means the frame is in the text's time interval, 0 means not.
    # means for each text, which frames are in its time interval.
    ...
    def __getitem__(self, item):
        dat = self.dat[item]
        pairs_text, pairs_mask, group_mask, ep_sentence = self._get_text(dat['sentences'])
        duration = dat['length']
        video, video_mask, video_path = self._get_rawvideo(dat['video'], duration, 0, duration)
        vt_mask = self._get_vt_mask(video_mask, duration, dat['start'], dat['end'])
        return pairs_text, pairs_mask, group_mask, video, video_mask, vt_mask, video_path, ep_sentence