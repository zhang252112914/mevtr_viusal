...
def prep_optimizer(args, model, num_train_optimization_steps, device, n_gpu, local_rank, coef_lr=1.):
    ...
    # clip related and non-related parameters in decay
    decay_clip_param_tp = [(n, p) for n, p in decay_param_tp if "clip." in n]
    decay_noclip_param_tp = [(n, p) for n, p in decay_param_tp if "clip." not in n]

    # clip related and non-related parameters in no decay
...
def eval_epoch(args, model, test_dataloader, device, n_gpu):
    ...
    with torch.no_grad():
        ...
        batch_video_paths, batch_video_captions = [], []
        # ----------------------------
        # 1. cache the features
        # ----------------------------
        for bid, batch in enumerate(test_dataloader):
            batch_tensors = tuple(t.to(device) for t in batch if torch.is_tensor(t))
            input_ids, input_mask, group_mask, video, video_mask, vt_mask = batch_tensors
            video_paths, video_captions = batch[-2], batch[-1]
            ...
            # 批次中的视频路径添加到全局视频路径列表
            batch_video_paths.extend(list(video_paths[0]))
            
            # 检查并添加非空字幕到全局字幕列表
            for video_idx, _ in enumerate(video_paths[0]):
                current_video_captions = []
                for caption_tuple in video_captions:  # 遍历每个视频的每个字幕
                    caption = caption_tuple[video_idx]
                    if caption != "":  # 只添加非空字幕
                        current_video_captions.append(caption) # add to current video captions list
                batch_video_captions.append(current_video_captions) # add to global video captions list
            
            # # Faster version?
            # batch_video_paths.extend(list(video_paths[0]))

            # # Preallocate memory for batch_video_captions
            # batch_video_captions = [[] for _ in range(len(video_paths[0]))]

            # # Use list comprehension to create current_video_captions
            # for video_idx, _ in enumerate(video_paths[0]):
            #     batch_video_captions[video_idx] = [caption_tuple[video_idx] for caption_tuple in video_captions if caption_tuple[video_idx] != ""]


            if (bid + 1) % args.n_display == 0:
                logger.info("Evaluation step: {}/{}".format(bid, len(test_dataloader)))
            end_cluster = time.time()
            print("cluster time: {}".format(end_cluster - start_cluster))
    ...
    tv_metrics, vt_metrics, ind_mask, ind_mask_t = compute_metrics_together(sim_matrix.T, sim_matrix_mask.T)
    ...
    save_information_on_local(args, metrics)
...
def save_information_on_local(args, metrics):
    logger.info("Saving the matrixs and metrics on local.")
    mat_save_path = os.path.join(args.output_dir, "sim_matrix_with_GT.mat")
    savemat(mat_save_path, metrics)
