...
from modules.cluster import get_deep_cluster, get_cluster_inter
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, video_frame=-1, visual=False):
        if not visual:
            ...
            return self.resblocks((x, video_frame))
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, video_frame=-1):
        ...
        # 	x = (x.reshape(B, video_frame, -1, self.width) +
        # 			self.frame_embedding.to(x.dtype)).reshape(B * video_frame, -1, self.width)
        ...
        x, _ = self.transformer(x, video_frame=video_frame, visual=True)
