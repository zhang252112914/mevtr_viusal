...
class CrossEnMulti_unbalanced(nn.Module):
    def __init__(self):
        super(CrossEnMulti_unbalanced, self).__init__()

    def forward(self, sim_matrix, sim_mask):
        sim_matrix = sim_matrix - (torch.max(sim_matrix, dim=0)[0] + torch.min(sim_matrix, dim=0)[0]) / 2.0
        expsim = torch.exp(sim_matrix)
        # sumexp = torch.sum(expsim, dim=0)
        logpt = torch.empty_like(sim_matrix)
        for i in range(len(sim_matrix)):
            # logpt[i] = expsim[i] / (sumexp - expsim[i])
            sum_mask = 1 - sim_mask
            sum_mask[i] = 1
            logpt[i] = expsim[i] / torch.sum(sum_mask*expsim, dim=0)
        # logpt = F.log_softmax(sim_matrix, dim=0)
        logpt = torch.log(logpt)
        ssim = torch.sum(sim_mask, dim=0)
        ssim[ssim==0] = -1
        sim_mask = sim_mask / ssim
        logpt = -torch.sum(logpt*sim_mask, dim=0)
        loss = logpt.mean()
        return loss
...
def all_gather_only(tensor, args, keep_itself=False):
    with torch.no_grad():
        if tensor.dtype == torch.bool:
            tensor_ = tensor.int()
        else:
            tensor_ = tensor
        all_x = [torch.empty_like(tensor_) for _ in range(args.world_size)]
        torch.distributed.all_gather(all_x, tensor_)

    if keep_itself:
        all_x[args.local_rank] = tensor

    if tensor.dtype == torch.bool:
        for i in range(args.world_size):
            all_x[i] = all_x[i].bool()
